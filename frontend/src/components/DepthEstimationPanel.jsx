...
function DepthStatsLine({ result }) {
  ...
  if (!result.available) return <div style={{ color: "#9ca3af" }}>{result.reason}</div>;
  ...
  return (
    <div style={sectionStyle}>
      <div style={bodyStyle}>
        ...
        {spectral.result && !spectral.result.available && <div style={{ color: "#9ca3af" }}>{spectral.result.reason}</div>}
        {spectral.result?.available && (
          <div style={{ color: "#374151" }}>평균 소스 심도: {spectral.result.depth_m.toFixed(1)} m</div>
        )}
      </div>
    </div>
  );
