...
const subHeaderStyle = { fontWeight: 600, color: "#374151", marginTop: 4 };
const hrStyle = { border: "none", borderTop: "1px solid #e5e7eb", margin: "4px 0" };
...
function RepeatabilitySection({
  ...
  return (
    <div>
      ...
      />
      {uploading && <div style={{ color: "#6b7280" }}>업로드 중...</div>}
      {uploadInfo && <div style={{ color: "#374151" }}>업로드됨: {uploadInfo.n_points}개 포인트</div>}
      ...
      {result && result.available && (
        <div style={{ color: "#374151" }}>
          ...
          {result.groups?.map((g, i) => (
            <div key={i} style={{ marginTop: 4, paddingLeft: 8, borderLeft: "2px solid #e5e7eb" }}>
  ...
  return (
    <div>
      ...
      {result && result.available && (
        <>
          <Plot
            data={[{ x: result.freqs_hz, y: result.psd, type: "scatter", mode: "lines", line: { color: "#2563eb" } }]}
            layout={{
              width: 280,
              height: 180,
          ...
          {result.peak_frequencies_hz?.length > 0 && (
            <div>
              ...
              {result.peak_frequencies_hz.map((f) => (
                <button
                  key={f}
                  style={{ ...buttonStyle, background: "white", color: "#2563eb", padding: "3px 6px", marginLeft: 4, marginTop: 4 }}
