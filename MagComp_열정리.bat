...
echo ============================================
echo   MagComp 열정리 - CK열로 밀린 값을 BK열로
echo ============================================
echo.
...
echo ------------------------------------------------------------
echo.
echo 위 내용대로 고쳐서 저장할까요?
echo 원본 파일은 그대로 두고 "원래이름-m.csv" 로 새로 만듭니다.
echo.
set "GO="
...
echo.
echo [참고] BK/CK 가 아닌 다른 열이었다면 열을 지정해 실행하세요.
echo        %PYEXE% "%SCRIPT%" "%TARGET%" --to-column BK --from-column CJ
echo.
echo        고칠 것이 없던 파일까지 전부 -m 이름으로 갖추고 싶다면:
...
echo          MagComp_열정리.bat "D:\다른경로\Magnetometer"
echo.
pause
exit /b 1

:noscript
echo [오류] fix_magcomp_column.py 를 찾을 수 없습니다.
